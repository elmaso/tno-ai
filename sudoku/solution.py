...
# we initializes the rows and columns
rows = 'ABCDEFGHI'
cols = '123456789'
...
# we build the sudoku pices
boxes = cross(rows, cols)
row_units = [cross(r, cols) for r in rows]
columns_units = [cross(rows, c) for c in cols]
square_units = [cross(rs, cs) for rs in ('ABC','DEF','GHI') for cs in ('123','456','789')]
unitlist = row_units + columns_units + square_units
units = dict((s, [u for u in unitlist if s in u]) for s in boxes)
peers = dict((s, set(sum(units[s],[]))-set([s])) for s in boxes)

# we implement diagonal sudokus
diag = [row[i] for i,row in enumerate(row_units)]
diag_rev =  [row[-i-1] for i,row in enumerate(row_units)]
...
if __name__ == '__main__':
    ...
    xx = dict(zip(boxes, diag_sudoku_grid))
